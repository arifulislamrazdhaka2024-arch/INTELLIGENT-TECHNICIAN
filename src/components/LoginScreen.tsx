    ...
    <div className={`min-h-screen w-full flex flex-col justify-between relative overflow-hidden ${getBackgroundStyle()} text-white select-none`}>
      ...
      <main className="w-full max-w-md mx-auto px-4 py-6 z-10">
        <div className="bg-slate-900/80 border border-slate-800 backdrop-blur-2xl rounded-3xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
          ...
          {selectedRole === 'EMPLOYEE' && employeeUsers.length > 0 && (
            <div className="mt-5 pt-4 border-t border-slate-800/80 text-center">
              <p className="text-[11px] text-slate-400">
                {language === 'bn' ? 'স্টাফ ডেমো লগইন আইডি:' : 'Staff Demo Login ID:'}{' '}
                <button
                  type="button"
                  onClick={() => {
                    setUserId(employeeUsers[0].userId);
                    setPassword(employeeUsers[0].password);
                    setSelectedRole('EMPLOYEE');
                  }}
                  className="text-indigo-400 font-mono font-bold hover:underline cursor-pointer"
                >
                  {employeeUsers[0].userId} (Pass: {employeeUsers[0].password})
                </button>
              </p>
            </div>
          )}
          ...
          {onGoToSetup && (
            <div className="mt-4 pt-3 border-t border-slate-800/60 text-center">
              <button
                type="button"
                onClick={onGoToSetup}
                className="text-[11px] text-slate-400 hover:text-amber-400 transition-colors underline cursor-pointer"
              >
                {language === 'bn'
                  ? 'মাস্টার সিইও অ্যাকাউন্ট সেটআপ স্ক্রিনে যান'
                  : 'Go to Master CEO Setup Screen'}
              </button>
            </div>
          )}
        </div>
