              ...
              <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => {
                      const link = `${window.location.origin}${window.location.pathname}#staff`;
                      navigator.clipboard.writeText(link);
                      setStaffLinkCopied(true);
                      setTimeout(() => setStaffLinkCopied(false), 3000);
                    }}
                    className="px-3 py-1.5 bg-indigo-600/30 hover:bg-indigo-600 text-indigo-300 hover:text-white border border-indigo-500/40 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
                    title="Copy direct staff login link to send via WhatsApp/SMS"
                  >
                    {staffLinkCopied ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Share2 className="w-3.5 h-3.5" />}
                    <span>{staffLinkCopied ? (language === 'bn' ? 'লিংক কপি হয়েছে!' : 'Link Copied!') : (language === 'bn' ? 'স্টাফ লিংক কপি' : 'Share Staff Link')}</span>
                  </button>
