...
export const InitialSetupModal: React.FC<InitialSetupModalProps> = ({ onComplete, onGoToLogin, language }) => {
  const [name, setName] = useState('Engineer MD Ariful Islam');
  const [userId, setUserId] = useState('');
  const [password, setPassword] = useState('');
  const [confirmPassword, setConfirmPassword] = useState('');
  const [mobile, setMobile] = useState('01977298036');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onComplete(ceoUser, {
      contacts: {
        ...
        primaryPhone: mobile.trim() || '01977298036',
        techSupportPhone: '0197729803688',
        emergencyHotline: '01977298036',
        whatsAppSupport: mobile.trim() || '01977298036',
        emailAddress: email.trim() || 'arifulislamrazdhaka2024@gmail.com',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-xl overflow-y-auto">
      <div className="relative w-full max-w-xl bg-slate-900 border border-blue-500/30 rounded-3xl shadow-[0_0_50px_rgba(30,58,138,0.35)] overflow-hidden my-8">
        ...
        {/* Role Quick Selector / Staff Login Switch */}
        {onGoToLogin && (
          <div className="bg-slate-950/90 px-6 py-3 border-b border-slate-800 flex items-center justify-between gap-3">
            <span className="text-xs text-slate-400 font-medium">
              {language === 'bn' ? 'আপনি কি স্টাফ বা টেকনিশিয়ান?' : 'Are you an employee / staff?'}
            </span>
            <button
              type="button"
              onClick={onGoToLogin}
              className="px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-indigo-600/30 flex items-center gap-1.5 cursor-pointer"
            >
              <span>{language === 'bn' ? 'স্টাফ লগইনে যান →' : 'Go to Staff Login →'}</span>
            </button>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-4">
          ...
          <div className="pt-3">
            <button
              type="submit"
              className="w-full py-3.5 px-6 bg-gradient-to-r from-blue-600 via-blue-500 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold text-sm rounded-xl shadow-lg shadow-blue-600/30 transition-all flex items-center justify-center gap-2 cursor-pointer"
            >
          ...
          {onGoToLogin && (
            <div className="text-center pt-2">
              <button
                type="button"
                onClick={onGoToLogin}
                className="text-xs text-slate-400 hover:text-indigo-400 transition-colors cursor-pointer underline underline-offset-4"
              >
                {language === 'bn'
                  ? 'আমি সিইও নই — স্টাফ বা কর্মচারী হিসেবে লগইন স্ক্রিনে যান'
                  : 'I am not the CEO — Go to Staff / Employee Login'}
              </button>
            </div>
          )}
        </form>
