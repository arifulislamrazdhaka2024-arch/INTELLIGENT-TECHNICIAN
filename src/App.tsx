...
export default function App() {
  ...
  // Allow switching directly to Staff Login if user is not CEO, or if URL contains #staff / ?role=staff
  const [forceLoginView, setForceLoginView] = useState<boolean>(() => {
    if (typeof window !== 'undefined') {
      const hash = (window.location.hash || '').toLowerCase();
      const search = (window.location.search || '').toLowerCase();
      return hash.includes('staff') || hash.includes('login') || search.includes('staff') || search.includes('login');
    }
    return false;
  });
  ...
  const handleInitialSetupComplete = (ceoUser: UserAccount, updatedConfig: Partial<SystemConfig>) => {
    ...
    // Keep existing staff accounts and add the newly created CEO
    const existingStaff = users.filter(u => u.role !== 'CEO');
    const updatedUsers = [ceoUser, ...existingStaff];
  ...
  // STEP 1: If system has never been configured and user has not switched to Staff Login
  if (!config.isInitialized && !forceLoginView) {
    return (
      <InitialSetupModal
        onComplete={handleInitialSetupComplete}
        onGoToLogin={() => setForceLoginView(true)}
  ...
  if (!currentUser) {
    return (
      <LoginScreen
        ...
        language={config.language}
        onGoToSetup={!config.isInitialized ? () => setForceLoginView(false) : undefined}
      />
    );
  }
