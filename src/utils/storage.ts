...
import { DEFAULT_SYSTEM_CONFIG, INITIAL_PRODUCTS, THEME_PALETTES, INITIAL_STAFF_USERS } from '../data/initialData';
...
export function loadUsers(): UserAccount[] {
  try {
    ...
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed) && parsed.length > 0 ? parsed : INITIAL_STAFF_USERS;
  } catch (e) {
